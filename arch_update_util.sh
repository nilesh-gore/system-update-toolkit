#!/bin/sh
# System Update Utility - Arch Linux / Manjaro / EndeavourOS
# A premium, robust script to keep your pacman-based Linux environment in top shape.
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        -h|--help)
            echo "Usage: sudo ./arch_update_util.sh [OPTIONS]"
            echo ""
            echo "Options:"
            ...
            echo ""
            echo "A premium system update utility for Arch Linux, Manjaro, and EndeavourOS."
            echo "Automates updates, cache cleanup, and disk recovery."
            exit 0
            ;;
        -v|--version)
            echo "System Update Utility (Arch) v$SCRIPT_VERSION"
            exit 0
            ;;
        -y|--yes)
            AUTO_YES=true
...
echo "${BOLD}${CYAN}**************************************************${NC}"
echo "${BOLD}${CYAN}*         Arch Linux System Update Utility       *${NC}"
echo "${BOLD}${CYAN}**************************************************${NC}"

# Check for pacman
if ! command -v pacman >/dev/null 2>&1; then
    echo "${RED}Error: pacman package manager not found. This script requires an Arch-based system.${NC}"
    exit 1
fi
...
# Capture pacman package cache size
PACMAN_CACHE_BEFORE=$(du -sb /var/cache/pacman/pkg 2>/dev/null | awk '{print $1}')
[ -z "$PACMAN_CACHE_BEFORE" ] && PACMAN_CACHE_BEFORE=0
...
echo "${BLUE}==>${NC} ${BOLD}Synchronizing repositories and upgrading system via pacman...${NC}"
if [ "$DRY_RUN" = true ]; then
    echo "${CYAN}[DRY RUN] Would run: sudo pacman -Syu --noconfirm${NC}"
else
    sudo pacman -Syu --noconfirm
fi

echo "${BLUE}==>${NC} ${BOLD}Checking for orphaned packages...${NC}"
ORPHANS=$(pacman -Qtdq 2>/dev/null || true)
if [ -n "$ORPHANS" ]; then
    if [ "$DRY_RUN" = true ]; then
        echo "${CYAN}[DRY RUN] Would run: sudo pacman -Rns \$(pacman -Qtdq) --noconfirm${NC}"
    else
        # Intentional word-splitting: $ORPHANS is a newline-separated list of package names
...
echo "${BLUE}==>${NC} ${BOLD}Cleaning up pacman package cache...${NC}"
if command -v paccache >/dev/null 2>&1; then
    if [ "$DRY_RUN" = true ]; then
        echo "${CYAN}[DRY RUN] Would run: sudo paccache -rk1${NC}"
    else
        sudo paccache -rk1
    fi
else
    echo "${YELLOW}'paccache' not found (install 'pacman-contrib' for smarter cache pruning). Falling back to 'pacman -Sc'.${NC}"
    if [ "$DRY_RUN" = true ]; then
        echo "${CYAN}[DRY RUN] Would run: sudo pacman -Sc --noconfirm${NC}"
    else
...
echo "Pacman cache cleared  : ${BOLD}$(numfmt --to=iec "$PACMAN_DIFF" 2>/dev/null || echo "$PACMAN_DIFF bytes")${NC}"
...
printf "\n${GREEN}%s - Arch Linux system update completed successfully.${NC}\n" "$(date)" | sudo tee -a /var/log/sysupdate.log
