#!/bin/sh
# System Update Utility - openSUSE (Leap/Tumbleweed)
# A premium, robust script to keep your zypper-based Linux environment in top shape.
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        -h|--help)
            echo "Usage: sudo ./opensuse_update_util.sh [OPTIONS]"
            echo ""
            echo "Options:"
            ...
            echo ""
            echo "A premium system update utility for openSUSE (Leap/Tumbleweed)."
            echo "Automates updates, cache cleanup, and disk recovery."
            exit 0
            ;;
        -v|--version)
            echo "System Update Utility (openSUSE) v$SCRIPT_VERSION"
            exit 0
            ;;
        -y|--yes)
            AUTO_YES=true
...
echo "${BOLD}${CYAN}**************************************************${NC}"
echo "${BOLD}${CYAN}*        openSUSE System Update Utility          *${NC}"
echo "${BOLD}${CYAN}**************************************************${NC}"

# Check for zypper
if ! command -v zypper >/dev/null 2>&1; then
    echo "${RED}Error: zypper package manager not found. This script requires an openSUSE-based system.${NC}"
    exit 1
fi
...
# Capture zypper package cache size
ZYPPER_CACHE_BEFORE=$(du -sb /var/cache/zypp 2>/dev/null | awk '{print $1}')
[ -z "$ZYPPER_CACHE_BEFORE" ] && ZYPPER_CACHE_BEFORE=0
...
echo "${BLUE}==>${NC} ${BOLD}Refreshing repositories and upgrading system via zypper...${NC}"
if [ "$DRY_RUN" = true ]; then
    echo "${CYAN}[DRY RUN] Would run: sudo zypper --non-interactive refresh && sudo zypper --non-interactive update${NC}"
else
    sudo zypper --non-interactive refresh
    sudo zypper --non-interactive update
fi

echo "${BLUE}==>${NC} ${BOLD}Checking for unneeded packages...${NC}"
UNNEEDED=$(zypper --non-interactive packages --unneeded 2>/dev/null | awk -F'|' 'NR>4 {gsub(/^[ \t]+|[ \t]+$/,"",$3); if ($3!="") print $3}') || true
if [ -n "$UNNEEDED" ]; then
    if [ "$DRY_RUN" = true ]; then
        echo "${CYAN}[DRY RUN] Would run: sudo zypper --non-interactive remove --clean-deps <unneeded packages>${NC}"
    else
        # Intentional word-splitting: $UNNEEDED is a newline-separated list of package names
        # shellcheck disable=SC2086
        sudo zypper --non-interactive remove --clean-deps $UNNEEDED || echo "${YELLOW}Some unneeded packages could not be removed.${NC}"
    fi
else
    echo "No unneeded packages found."
fi

echo "${BLUE}==>${NC} ${BOLD}Cleaning up zypper package cache...${NC}"
if [ "$DRY_RUN" = true ]; then
    echo "${CYAN}[DRY RUN] Would run: sudo zypper --non-interactive clean --all${NC}"
else
...
ZYPPER_CACHE_AFTER=$(du -sb /var/cache/zypp 2>/dev/null | awk '{print $1}')
[ -z "$ZYPPER_CACHE_AFTER" ] && ZYPPER_CACHE_AFTER=0
...
echo "Zypper cache cleared  : ${BOLD}$(numfmt --to=iec "$ZYPPER_DIFF" 2>/dev/null || echo "$ZYPPER_DIFF bytes")${NC}"
...
printf "\n${GREEN}%s - openSUSE system update completed successfully.${NC}\n" "$(date)" | sudo tee -a /var/log/sysupdate.log
